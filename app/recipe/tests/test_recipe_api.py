...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_recipe_list_limited_to_user(self):
        """Test list of recipes is limited to authenticated user's ones."""
    ...
    def test_get_recipe_detail(self):
        ...
        url = get_recipe_url(recipe_id=recipe.id)
    ...
    def test_full_update(self):
        """Test full update of a recipe (PUT)."""
        recipe = create_recipe(
            user=self.user,
            title='Sample recipe title',
        ...
        payload = {
            ...
            'link': 'https://example.com/rnew-ecipe.pdf',
    ...
    def test_delete_other_users_recipe_error(self):
        """Test trying to delete other users' recipes gives error."""
        new_user = create_user(email='newuser@example.com', password='test123')
    ...
    def test_create_recipe_with_existing_tags(self):
        ...
        # Test that the already-existing tag was used, not a new one
    ...
    def test_create_recipe_with_existing_ingredients(self):
        ...
        self.assertEqual(recipe.ingredients.count(), 2)
        for ingredient_lemon in payload['ingredients']:
            exists = recipe.ingredients.filter(
                name=ingredient_lemon['name'], user=self.user
            ).exists()
            self.assertTrue(exists)
    ...
    def test_update_recipe_assign_ingredient(self):
        ...
        ingredient_banana = Ingredient.objects.create(
            user=self.user, name='Bananas'
        )

        payload = {'ingredients': [{'name': 'Bananas'}]}

        url = get_recipe_url(recipe.id)
        res = self.client.patch(url, payload, format='json')

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertIn(ingredient_banana, recipe.ingredients.all())
